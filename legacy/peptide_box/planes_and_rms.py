...
def fit_subplane_CNO(at_i, at_j, ca_center, ca_normal):
    """
    Fit a plane to C'(i), N(j), O(i) and compute:
      - subplane normal
      - RMS of CNO atoms to the Cα plane
    """
    Ci = at_i.get("C")
    Oi = at_i.get("O")
    Nj = at_j.get("N")
    if Ci is None or Oi is None or Nj is None:
        return None

    pts = [_v(Ci), _v(Nj), _v(Oi)]
    c_sub, n_sub, _ = fit_plane(pts)

    # RMS of CNO atoms to the Cα plane
    dists = [
        np.dot((_v(Ci) - ca_center), ca_normal),
        np.dot((_v(Nj) - ca_center), ca_normal),
        np.dot((_v(Oi) - ca_center), ca_normal),
    ]
    rms_to_ca = float(np.sqrt(np.mean(np.square(dists))))

    return dict(
        sub_center=tuple(c_sub),
        sub_normal=tuple(normalize(n_sub)),
        rms_to_ca=rms_to_ca,
    )
...
def build_plane_for_pair(at_i, at_j, pads) -> Optional[dict]:
    ...
    sub = fit_subplane_CNO(at_i, at_j, c, n)
    ...
    plane["cno_normal"] = sub["sub_normal"] if sub else None
    plane["cno_rms"] = sub["rms_to_ca"] if sub else None
...
def write_normals_csv(base: str, planes: List[dict]) -> str:
    ...
    with open(path, "w", newline="") as g:
        ...
        w.writerow([
        "plane_index","chain",
        "res_i","icode_i","resname_i",
        "res_j","icode_j","resname_j",
        "nx","ny","nz","rms",
        "cno_nx","cno_ny","cno_nz","cno_rms"
    ])
        for k, pl in enumerate(planes, start=1):
            nx, ny, nz = [float(pl["normal"][0]), float(pl["normal"][1]), float(pl["normal"][2])]
            rms = float(pl["rms"])
            cno = pl.get("cno_normal")
            cno_nx, cno_ny, cno_nz = cno if cno else (None, None, None)

            w.writerow([
                k,
                pl["chain"].strip(),
                pl["resseq_i"], pl["icode_i"], pl["resname_i"],
                pl["resseq_j"], pl["icode_j"], pl["resname_j"],
                f"{nx:.6f}", f"{ny:.6f}", f"{nz:.6f}",
                f"{rms:.6f}",
                f"{cno_nx:.6f}" if cno_nx is not None else "",
                f"{cno_ny:.6f}" if cno_ny is not None else "",
                f"{cno_nz:.6f}" if cno_nz is not None else "",
                f"{pl.get('cno_rms', 0.0):.6f}"
            ])
    return path

...
from scripts.run_two_class_peptide_plane_prototype import (
    PERTURBED_ATOM_NAMES,
    PrototypeSpec,
    best_score_rows,
    build_report_text,
    class_for_chain,
    ...
    prototype_specs,
    should_perturb_atom,
    variant_id_for_angles,
    write_prototype_variant,
)
...
def test_class_assignment_and_variant_ids() -> None:
    ...
    assert variant_id_for_angles(2.0, -4.0) == "two_class_trip2_cym4"
    assert len(prototype_specs([-2.0, 0.0, 2.0])) == 9
...
def test_no_change_reference_preserves_coordinates_and_identity(tmp_path: Path) -> None:
    ...
    write_prototype_variant(lines, atoms, PrototypeSpec("ref", 0.0, 0.0), out)
...
def test_class_specific_perturbation_moves_only_intended_conservative_atom_set(tmp_path: Path) -> None:
    parent = tmp_path / "parent.pdb"
    out = tmp_path / "tri_only.pdb"
    write_fixture(parent)
    lines, atoms = parse_pdb_atom_lines(parent)

    write_prototype_variant(lines, atoms, PrototypeSpec("tri_only", 4.0, 0.0), out)
    _out_lines, variant_atoms = parse_pdb_atom_lines(out)
    ...
    assert {atom.atom_name for atom in moved} <= PERTURBED_ATOM_NAMES
    assert all(atom.atom_name not in {"CA", "OE1"} for atom in moved)
    assert all(not should_perturb_atom(atom) or atom.chain in {"A", "C", "E"} or atom.atom_name not in PERTURBED_ATOM_NAMES for atom in fixed)


def test_threefold_class_organization_is_preserved_for_ca_anchors(tmp_path: Path) -> None:
    parent = tmp_path / "parent.pdb"
    out = tmp_path / "both.pdb"
    write_fixture(parent)
    lines, atoms = parse_pdb_atom_lines(parent)
    write_prototype_variant(lines, atoms, PrototypeSpec("both", 4.0, -4.0), out)
    _out_lines, variant_atoms = parse_pdb_atom_lines(out)

    parent_ca = [atom for atom in atoms if atom.atom_name == "CA"]
    variant_ca = [atom for atom in variant_atoms if atom.atom_name == "CA"]

    assert [atom.chain for atom in parent_ca] == [atom.chain for atom in variant_ca]
    assert all(np.allclose(before.coord, after.coord) for before, after in zip(parent_ca, variant_ca))
...
def test_report_wording_contains_required_cautions() -> None:
    scores = pd.DataFrame(
        {
            "variant_id": ["two_class_tri0_cy0", "two_class_trip2_cym2"],
            "triamino_tilt_deg": [0.0, 2.0],
            "triketo_tilt_deg": [0.0, -2.0],
            "C_peak_A": [5.745, 5.7],
    ...
    geometry = pd.DataFrame(
        {
            "variant_id": ["two_class_trip2_cym2"],
            "row_type": ["summary"],
    ...
    assert "controlled prototype transform" in text
    assert "not a final atomistic reconstruction" in text
    assert "two separate three-fold peptide/backbone degrees of freedom" in text

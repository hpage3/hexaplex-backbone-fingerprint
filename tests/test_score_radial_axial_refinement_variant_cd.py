...
from scripts.score_radial_axial_refinement_variant_cd import (
    add_relative_scores,
...
def test_identifies_baseline_control_1p0000_1p0000() -> None:
    scores = pd.DataFrame(
        {
            "variant_id": ["other", "baseline"],
            "radial_scale_xy": [1.0025, 1.0],
            "axial_scale_z": [0.995, 1.0],
        }
    )
...
def test_pivot_table_data_for_c_d_and_combined_error() -> None:
    scores = pd.DataFrame(
        {
            "variant_id": ["a", "b", "c", "d"],
            "radial_scale_xy": [1.0, 1.01, 1.0, 1.01],
            "axial_scale_z": [0.99, 0.99, 1.0, 1.0],
            "C_peak_A": [5.6, 5.7, 5.8, 5.9],
            "D_peak_A": [7.1, 7.2, 7.3, 7.4],
            "combined_abs_error_A": [0.4, 0.3, 0.2, 0.1],
        }
    )
    c_pivot = pivot_table_data(scores, "C_peak_A")
    d_pivot = pivot_table_data(scores, "D_peak_A")
    e_pivot = pivot_table_data(scores, "combined_abs_error_A")
    assert c_pivot.loc[0.99, 1.0] == 5.6
    assert d_pivot.loc[1.0, 1.01] == 7.4
    assert e_pivot.loc[1.0, 1.01] == 0.1


def test_report_text_contains_required_cautions() -> None:
    scores = pd.DataFrame(
        {
            "variant_id": ["a", "baseline"],
            "radial_scale_xy": [1.0, 1.0],
            "axial_scale_z": [0.99, 1.0],
    ...
    assert "focused radial/axial refinement" in text

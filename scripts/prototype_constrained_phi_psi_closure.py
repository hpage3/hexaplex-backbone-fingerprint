...
def build_closure_window(residues: list[Residue], chain_id: str, start_index: int) -> ClosureWindow:
    ...
    required = {"N", "CA", "C", "O"}
    for residue in [prev_res, first, second]:
        ...
        rows.append(
            {
                ...
                "solved_torsion_1_delta_deg": solved_delta,
                "solved_torsion_2_name": "",
                "solved_torsion_2_delta_deg": np.nan,
...
def write_report(results: pd.DataFrame, path: Path) -> None:
    ...
    summary = results.groupby(["chain_id", "residue_names", "fixed_torsion_name", "notes"], as_index=False).agg(
...
This is a small feasibility prototype, not the full systematic torsion search. It keeps CA anchors fixed, keeps omega fixed to the baseline trans-like value, perturbs one torsion, and solves the other torsion using `{method}`.
...
{markdown_table(summary, ['chain_id', 'residue_names', 'fixed_torsion_name', 'notes', 'attempts', 'successes', 'median_error_A', 'max_error_A'])}

## Interpretation

- Can small phi/psi perturbations be geometrically closed with fixed CA anchors? See success counts above; this prototype checks one fixed torsion plus one solved torsion.
- Which torsions are best treated as fixed versus solved? In this two-CA endpoint model, `phi0_deg` and `psi0_deg` are the active pair. Fixing one and solving the other is the minimal closure test while omega is fixed.
...
- Full scan next step: add a two-torsion solve mode, explicit anti-parallel chain direction metadata, steric/geometry filters, then pass accepted candidate coordinates through the existing Debye/pair-family scoring scripts.
"""
...
def run_prototype(
    ...
    for window in windows:
        rows.extend(run_window_closure(window, fixed_torsion_name, perturbations, tolerance_A, omega_mode=omega_mode))

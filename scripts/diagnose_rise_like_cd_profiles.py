"""Diagnose local C/D profiles for rise-like variants."""
...
from scripts.score_rise_like_variant_cd import BASELINE_RISE_SCALE, baseline_row, sort_by_rise


DEFAULT_SCORE_CSV = Path("outputs/metrics/rise_like_variant_cd_scores.csv")
DEFAULT_OUT_CSV = Path("outputs/metrics/rise_like_cd_profile_diagnostics.csv")
DEFAULT_REPORT = Path("outputs/reports/rise_like_cd_profile_diagnostics.md")
...
def diagnostic_band_row(
    ...
    """Build diagnostics for one rise-like variant and one band."""
    ...
    return {
        ...
        "axial_rise_scale": score_row["axial_rise_scale"],
        "band": band,
...
def run_diagnostics(
    ...
    """Run rise-like local profile diagnostics and write outputs."""
...
def build_report_text(diagnostics: pd.DataFrame, scores: pd.DataFrame, baseline: pd.Series) -> str:
    """Build rise-like profile diagnostic report."""
    cols = [
        "variant_id",
        "axial_rise_scale",
        "band",
    ...
    return f"""# Rise-Like Local C/D Profile Diagnostics

## Purpose

This diagnostic checks whether the C and D local profiles move smoothly across the explicit rise-like branch, using the uncompressed parent-like variant as the baseline.

- Baseline variant: `{baseline['variant_id']}`
...
C moves smoothly if centroid/parabolic estimates progress across the rise-like scale even when picked peaks remain binned. D remains position-stable if the picked peak and local centroid/parabolic estimates stay near the baseline while C shifts. These variants are diagnostic rise-like proxies, not minimized structures.
"""
...
def main() -> int:
    ...
    print(f"Generated {len(diagnostics)} rise-like local profile diagnostic rows")

"""Audit axial-only extension variants with global sanity gates."""
...
DEFAULT_OUT_CSV = Path("outputs/metrics/axial_only_extension_variant_geometry_audit.csv")
DEFAULT_REPORT = Path("outputs/reports/axial_only_extension_variant_geometry_audit.md")


def audit_variant(parent_atoms, manifest_row: pd.Series) -> dict[str, object]:
    """Audit one axial-only variant."""
...
def run_audit(manifest_path: Path, source_pdb: Path, out_csv: Path, report_path: Path) -> pd.DataFrame:
    """Run axial-only geometry audit and write outputs."""
...
def build_report_text(results: pd.DataFrame, source_pdb: Path) -> str:
    """Build axial-only geometry audit report."""
    total = len(results)
    ...
    return f"""# Axial-Only Extension Variant Geometry Audit

This audit uses the same global diagnostic sanity gates as the radial/axial refinement branch. These gates are not energy minimization.
...
def main() -> int:
    ...
    print(f"Audited {len(results)} axial-only extension variants")

"""Audit repeated GLU->MEP baseline-parent omega variants before diffraction scoring."""
...
from scripts.audit_constrained_phi_psi_candidate_geometry import Atom, atom_map, audit_candidate, distance, parse_pdb
...
DEFAULT_OUT_CSV = Path("outputs/metrics/repeated_glu_mep_baseline_omega_variant_geometry_audit.csv")
DEFAULT_REPORT = Path("outputs/reports/repeated_glu_mep_baseline_omega_variant_geometry_audit.md")
...
def classify_audit(audit: dict[str, object]) -> tuple[bool, str]:
    """Return safe/fail classification and failure reason string."""
    safe = safe_for_diffraction(audit)
    return safe, ";".join(failure_reasons(audit))


def audit_variant(parent_atoms: list[Atom], variant_path: Path) -> dict[str, object]:
    """Audit one GLU->MEP baseline-parent repeated variant."""
...
def join_manifest_audit(manifest_row: pd.Series, audit: dict[str, object]) -> dict[str, object]:
    """Merge manifest metadata and geometry audit results."""
    ...
    return {
        ...
        "solve_mode": manifest_row.get("solve_mode", ""),
        "omega_mode": manifest_row.get("omega_mode", ""),
...
def build_report_text(results: pd.DataFrame, source_pdb: Path) -> str:
    ...
    ) if total else "_No variants audited._"
    fail_text = markdown_table(
        failed[["variant_id", "fixed_torsion_delta_deg", "failure_reason"]]
    ) if not failed.empty else "_No variants failed the audit._"
    return f"""# Repeated GLU->MEP Baseline-Omega Variant Geometry Audit

This audit compares repeated GLU->MEP `baseline_parent` omega variants against the source backbone-plus-carboxylate model before diffraction scoring. The omega mode is `baseline_parent`, not `fixed_180`; this is a baseline-parent-omega pilot, not an unconstrained omega scan.

- Source PDB: `{source_pdb}`
- Repeated GLU->MEP baseline-omega variants audited: {total}
...
The audit verifies omega remains trans-like under the parent-baseline omega mode. It does not require omega to be exactly 180 degrees.
...
def run_audit(manifest_path: Path, source_pdb: Path, out_csv: Path, report_path: Path) -> pd.DataFrame:
    """Run geometry audit and write outputs."""
...
def main() -> int:
    ...
    print(f"Audited {len(results)} repeated GLU->MEP baseline-omega variants")
